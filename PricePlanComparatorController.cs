...
namespace JOIEnergy.Controllers
{
    ...
    public class PricePlanComparatorController : Controller
    {
        ...
        private readonly IGetCheapestPriceplanService _getCheapestPriceplanService;
        private readonly List<PricePlan> _pricePlans;

        public PricePlanComparatorController(IPricePlanService pricePlanService, IAccountService accountService, IGetCheapestPriceplanService getCheapestPriceplanService, List<PricePlan> pricePlans)
        {
            this._pricePlanService = pricePlanService;
            this._accountService = accountService;
            this._getCheapestPriceplanService = getCheapestPriceplanService;
            this._pricePlans = pricePlans;
        }
        ...
        [HttpGet("cheapest-plan/{smartMeterId}")]

        public ObjectResult GetCheapestplan(string smartMeterId)
        {
            if(_accountService.GetPricePlanIdForSmartMeterId(smartMeterId) == null)
            {
                return new NotFoundObjectResult(string.Format("Smart Meter ID not found", smartMeterId));
            }

            return new ObjectResult(_getCheapestPriceplanService.CheapestPriceplan(smartMeterId, _pricePlans));
        }
    }
}
